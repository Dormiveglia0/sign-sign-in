var t = require("../../@babel/runtime/helpers/typeof");
(wx.webpackJsonp = wx.webpackJsonp || []).push([
  [63], {
    2053: function(t, e, n) {
      n(581)
    },
    2054: function(t, e, n) {},
    3400: function(e, n, o) {
      o.r(n), o(2053);
      n = o(0);
      var a = o.n(n),
        r = (n = o(4), n = o.n(n), o(2054), function(t, e, n) {
          return e && i(t.prototype, e), n && i(t, n), t
        });

      function i(t, e) {
        for (var n = 0; n < e.length; n++) {
          var o = e[n];
          o.enumerable = o.enumerable || !1, o.configurable = !0, "value" in o && (o.writable = !0), Object.defineProperty(t, o.key, o)
        }
      }

      function u(e, n) {
        if (e) return !n || "object" != t(n) && "function" != typeof n ? e : n;
        throw new ReferenceError("this hasn't been initialised - super() hasn't been called")
      }

      function c() {
        var t, e;
        ! function(t, e) {
          if (!(t instanceof e)) throw new TypeError("Cannot call a class as a function")
        }(this, c);
        for (var n = arguments.length, o = Array(n), a = 0; a < n; a++) o[a] = arguments[a];
        return (t = e = u(this, (e = c.__proto__ || Object.getPrototypeOf(c)).call.apply(e, [this].concat(o)))).$usedState = ["itemDataNew", "coverImgDefault"], e.config = {
          component: !0
        }, e.customComponents = [], u(e, t)
      }
      r = n()({
        properties: {
          itemData: {
            type: Object,
            value: ""
          },
          itemIndex: {
            type: Number,
            value: ""
          }
        },
        data: {
          itemDataNew: "",
          coverImgDefault: "data:image/png;base64,"
        },
        ready: function() {
          this.setData({
            itemDataNew: this.data.itemData
          })
        },
        methods: {
          toCourseDetail: function(t) {
            t = t.currentTarget.dataset.item, (this.data.taskComplete || 31 != t.id && 18 != t.id) && (a.a.eventCenter.trigger("taroClick", {
              funName: "跳转课程详情",
              itemId: t.id
            }), a.a.navigateTo({
              url: "/secondBag/pages/courseDetails/courseDetails?id=" + t.id + "&title=" + t.title
            }))
          },
          listImgError: function(t) {
            var e;
            (t = t.target.dataset.index) && ((e = {})["itemDataNew.imgList[" + t + "]"] = "data:image/png;base64,", this.setData(e))
          }
        }
      })((function(e, n) {
        if ("function" != typeof n && null !== n) throw new TypeError("Super expression must either be null or a function, not " + t(n));
        e.prototype = Object.create(n && n.prototype, {
          constructor: {
            value: e,
            enumerable: !1,
            writable: !0,
            configurable: !0
          }
        }), n && (Object.setPrototypeOf ? Object.setPrototypeOf(e, n) : e.__proto__ = n)
      }(c, a.a.Component), r(c, [{
        key: "_constructor",
        value: function(t) {
          (function t(e, n, o) {
            null === e && (e = Function.prototype);
            var a = Object.getOwnPropertyDescriptor(e, n);
            return void 0 !== a ? "value" in a ? a.value : void 0 !== (a = a.get) ? a.call(o) : void 0 : null !== (a = Object.getPrototypeOf(e)) ? t(a, n, o) : void 0
          })(c.prototype.__proto__ || Object.getPrototypeOf(c.prototype), "_constructor", this).call(this, t), this.$$refs = new a.a.RefsArray
        }
      }, {
        key: "_createData",
        value: function() {
          var t = this,
            e = (n = (this.__state = arguments[0] || this.state || {}, this.__props = arguments[1] || this.props || {}, this.$prefix, this.data)).itemDataNew,
            n = n.coverImgDefault;
          return this.anonymousFunc0 = function(e) {
            e.stopPropagation(), t.toCourseDetail(e)
          }, Object.assign(this.__state, {
            itemDataNew: e,
            coverImgDefault: n
          }), this.__state
        }
      }, {
        key: "anonymousFunc0",
        value: function(t) {
          t.stopPropagation()
        }
      }]), r = n = c, n.$$events = ["anonymousFunc0"], n.$$componentPath = "components/item-course/item-course", n = r)) || n, Component(o(0).default.createComponent(r))
    },
    581: function(t, e, n) {
      t.exports = n.p + "components/item-course/item-course.wxml"
    }
  },
  [
    [3400, 0, 2, 1]
  ]
]);